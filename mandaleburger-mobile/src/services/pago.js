...
export const crearPreferenciaPago = async (planId) => {
    try {
        const res = await api.post(ENDPOINTS.CREAR_PREFERENCIA_SUSCRIPCION, { plan_id: planId, mobile: true });
        return res.data;
    } catch (err) {
...
export const crearPreferenciaPagoOrden = async (orderId) => {
    try {
        const res = await api.post(ENDPOINTS.CREAR_PREFERENCIA_ORDEN, { order_id: orderId, mobile:true });
        return res.data;
    } catch (err) {
