...
const handleArmarBurger = async (e) => {
  ...
  try {
    ...
    await handleCreateCustomerBurger(fd);

    toast.success("¡Burger creada correctamente!", { autoClose: 2000 });
    clearKeys(items);
    setNombre(""); setDescripcion("");
    setSelAderezos([]); setSelPapas([]); setSelBebidas([]);
    setIngBu([]); setTotalPrice("0.00");
    setTimeout(() => navigate("/client/carrito"), 3000);
  } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex justify-center items-start gap-6 py-6">
      <div className="bg-[#1C1C1C] p-6 rounded-lg shadow-lg w-1/3 max-w-2xl">
        <Carrusel 
        ref={carruselRef}
        panes={panes}
