...
import { useAsociarPromoPlan } from "../../hooks/useAsociarPromoPlan";
...
const AdminPromocionForm = () => {
    const navigate = useNavigate();
    const { ingredientes: categorias, cargando: cargandoIng, error: errorIng } = useIngredientes();
    const { handleCreatePromotion, cargando: cargandoPromo } = useCreatePromotion();
    const { handleAsociarPromo, cargando: cargandoAsociacion } = useAsociarPromoPlan();
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const promoCreada = await handleCreatePromotion(dataToSend);

            if (formData.plan) {
                await handleAsociarPromo({
                    promotion_id: promoCreada.id,
                    subscription_id: parseInt(formData.plan),
                });
            }

            toast.success("Promoción creada con éxito", { autoClose: 3000 });
            setFormData({ name: "", description: "", price: "", img: null, plan: "" });
            setSeleccionados({});
            setOpenCategories({});
        } catch (err) {
            toast.error("Error al crear la promoción", { autoClose: 3000 });
            console.error(err);
        }
    };

    if (cargandoIng || cargandoPlanes) {
        return (
    ...
    return (
        <div className="w-full max-w-md mx-auto mt-6 pb-25">
            ...
            <h1 className="text-2xl md:text-3xl font-bold mb-6">Crear promoción</h1>
            ...
            <form onSubmit={handleSubmit} className="w-full flex flex-col gap-4">
                ...
                <Button
                    ...
                    disabled={cargandoPromo || cargandoAsociacion}
                >
                    {(cargandoPromo || cargandoAsociacion) ? <Spinner /> : "Crear promoción"}
                </Button>
            </form>
...
export default AdminPromocionForm;
